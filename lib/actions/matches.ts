...
export async function getPotentialMatches(): Promise<UserProfile[]> {
  ...
  const adopterPreference = currentUserPrefs?.adopter_preference || "pet";
  const breedPreference = currentUserPrefs?.breed_preference || "";
  const genderPreference = currentUserPrefs?.gender_preference || [];
  const filteredMatches =
    potentialMatches
      .filter((match) => {
        if (match.adopter !== adopterPreference) {
          return false;
        }

        if (breedPreference.length > 0 && match.breed !== breedPreference) {
          return false;
        }
...
export async function getUserMatches() {
  ...
  for (const match of matches || []) {
    ...
    matchedUsers.push({
      ...
      breed: otherUser.breed,
      adopter: otherUser.adopter,
